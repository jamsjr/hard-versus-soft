...
\begin{abstract}
  ...
  \nota{Dizer que: RBS é útil, existe a distinção hard vs soft, não
    existe comparação, descobrimos que soft é melhor em quase todos os
    casos.}
...
server also maintais internally a tuple $(c,d)$, where $c$ is its
...
\item Whenever the server is scheduling a new job for execution, it
  ...
  $C$. Otherwise, the deadline $d$ and the capacity $c$ are unchanged.
...
constantly overloadedloaded or the server is underloaded both
...
In the first scenario, L1, . On the other examples, though, due to their time
variability, we found better to present graphs for ``Response time
$\times$ time'' (or, equivalently, ``Wait time $\times$ time''). On
the one hand, these graphs do not clearly show the distribution of the
results, but, on the other hand, they highlight the time variability
aspect, which we found important.
...
and were ommitted from this paper. As can be easily seen, in the
...
only missed 0.1\% of the dealines, while the hard reservation server
...
significantly smaller deadline miss ratios. This results sugests that
soft reservation servers iare better able to use system resources
